...
- The next PR runs confirmed lint and typecheck pass, then exposed the shared runtime failure: Vitest could not resolve `@/lib/utils` because `frontend_new/src/lib/utils.ts` was present only in the local ignored working tree. The root `.gitignore` pattern `lib/` also matched that source directory. Track the utility and explicitly unignore `frontend_new/src/lib/**` so clean CI checkouts contain it.
