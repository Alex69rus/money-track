...
- The next PR run confirmed lint and typecheck pass, then exposed the shared runtime cause: Vitest could not resolve `@/lib/utils`. Resolve Vite's `@` alias from the command's stable `frontend_new` working directory rather than the config module URL, which may point to Vite's temporary bundled-config location on GitHub runners.
